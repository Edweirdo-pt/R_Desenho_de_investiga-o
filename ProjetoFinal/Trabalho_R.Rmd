...
# Pergunta 2.Caracterize as variáveis Age, Gender, Blood.Type, Medical.Condition, recorrendo
a gráficos e/ou estatísticas descritivas de acordo com as características das variáveis. 
...
mtext(expression(bold("Gráfico 3 – ") * "Gráfico de barras da Distribuição por Sexo (Frequência e %)"),
      side = 1, line = 4)
...
mtext(expression(bold("Gráfico 4 – ") * "Gráfico de barras 'Prevalência por Grupo Sanguíneo'"),
      side = 1, line = 4)
...
# 6. Colocar Legenda no Gráfico
mtext(expression(bold("Gráfico 5 – ") * "Gráfico de barras 'Distribuição de Condições Médicas'"),
      side = 1, line = 4)
...
# Pergunta 3. Compare a variabilidade do número de dias que os pacientes estão internados (obtenha variável
secundária a partir da informação disponível na BD) de acordo com o tipo de admissão. 
...
# Pergunta 4. Compare os boxplots da idade (Age) dos pacientes nas diferentes categorias da condição médica
(Medical.Condition).
...
ggplot(amostra, aes(x = Medical.Condition, y = Age, fill = Medical.Condition)) +
  ...
  labs(key = "Os números na base indicam o tamanho da amostra (n) por categoria",
       x = "Condição Médica",
       y = "Idade (Anos)",
       caption = "Gráfico 7 - Boxplot da distribuição 'idade' vs 'Condição médica'") +
...
# Pergunta 5. Obtenha uma representação gráfica que represente em simultâneo as variáveis
Medical.Condition e Admission.Type. 
...
ggplot(amostra, aes(x = Medical.Condition, fill = Admission.Type)) +
  ...
  labs(x = "Condição Médica",
       ...
       fill = "Tipo de Admissão",
        caption = "Gráfico 8 - Tipo de Admissão por Condição Médica") +
...
# Pergunta 6. Para os pacientes que tiveram um resultado inconclusivo durante um teste médico na admissão,
caracterize a distribuição do valor faturado pelos serviços de saúde (Billing.Amount)
...
# Pergunta 7. Considere os pacientes do sexo masculino e que tomam aspirina ou ibuprofeno e obtenha uma
estimativa pontual e intervalar a 98% da média do número de dias internado (variável obtida em 3). 
...
# Pergunta 8. Assumindo a independência das observações e recorrendo ao Teorema do Limite Central, foi construído um intervalo de confiança a 98% para a média populacional do número de dias de internamento. O intervalo obtido foi [14.96 ; 16.18] dias, indicando que, com um nível de confiança de 98%, a média populacional se encontra compreendida neste intervalo.
...
# Pergunta 10. Considerando apenas os pacientes que tiveram os resultados dos exames na admissão normal ou
anormal, verifique se existe alguma associação entre o sexo e o resultado do teste. 
...
# Pergunta 11. Verifique se existe alguma correlação entre a idade e o número de dias internado. Construa o diagrama
de dispersão e use cores diferentes para identificar o sexo masculino e feminino.
...
ggplot(amostra, aes(x = Age, y = Days_Stay, color = Gender)) +
  ...
  labs(title = "",
       ...
       color = "Sexo",
       caption = "Gráfico 9 - Relação entre Idade e Dias de Internamento") +
  theme_minimal()
```
...
# Pergunta 12. Simule uma variável que represente a Idade para 150 pacientes, com base nas características da Idade
desta BD. Compare os resultados obtidos representando no mesmo gráfico as duas distribuições. 
...
ggplot(df_total, aes(x = Idade, fill = Tipo)) +
  ...
  labs(title = "",
       ...
       y = "Densidade",
       fill = "Distribuição",
       caption = "Gráfico 10 - Comparação de Distribuições: Idade Real vs. Simulada") +
  theme_minimal()
