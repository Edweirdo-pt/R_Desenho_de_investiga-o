...
library(ggplot2)
Sys.setlocale("LC_CTYPE", "Portuguese_Portugal.utf8")
...
*Tabela 1 - Classificação de variáveis em estudo*
...
*2.1 "Age"*
**Classificação:** Variável Quantitativa Discreta, medida numa escala de razão.

**Justificação:** Por se tratar de uma variável quantitativa discreta, a sua caracterização baseia-se em medidas de tendência central e de dispersão. O histograma permite observar a forma geral da distribuição, enquanto o boxplot possibilita a identificação de valores extremos e a análise da dispersão.
...
teste_10 <- chisq.test(table(dados_10$Gender, dados_10$Test.Results),correct=FALSE)
...
# organizar tabela 
  ...
  caption = "Média e desvio-padrão do valor faturado por tipo de admissão",
  align = "c"
)
```
